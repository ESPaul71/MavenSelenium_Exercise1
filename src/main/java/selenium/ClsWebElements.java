 ...
 */
    ...
    public boolean ForEachList(final By by, String pValue, String item) {
		List<WebElement> myElements = getWebList(by);
		for(WebElement e : myElements) {
			  System.out.println(e.getAttribute(pValue));
			  System.out.println(item);
			  String text = e.getAttribute(pValue);
			  if(text == item) {
				  ClsReport.fnLog(Status.PASS, "Product: " + e.getAttribute(pValue) + " encontrado", false); //Send product's name to report
				  System.out.println(e.getAttribute(pValue));
				  return true;
			  }
			}
